...
def create_nodes_and_links(G, n_nodes, n_topics):
    ...
    for author in range(n_nodes):
        ...
        for reverse in range(G.nodes):
            chance = 0.9 if nodes[author]['topic'] == nodes[reverse]['topic'] else 0.
...
def create_coauthor_graph(path_to_edglist = None,depth_graphs = 2):
    ...
    nx.write_edgelist(G, f'../profiles/graphs/{author} level {0}.csv', data = False,delimiter=';')
        ...
        for new_author in new_authors:
            if new_author not in done_authors:
                try:
                    ...
                    pass
        nx.write_edgelist(G, f'../profiles/graphs/{author} level {level+1}.csv', data = False,delimiter=';')        
